...
__all__ = ['InternalApplyModify','InternalApplyList','ApplyModifyWizard','ApplyListExhibition']

####################     请领单修改     ################################

class InternalApplyModify(ModelView):
    'Internal Apply Modify'

    __name__ = 'hrp_internal_delivery.internal_apply_modify'
    ...
    moves = fields.One2Many('hrp_internal_delivery.apply_list_exhibition','None','moves',domain=[
...
class ApplyListExhibition(ModelView):
    'Apply List Exhibition'
    __name__ = 'hrp_internal_delivery.apply_list_exhibition'
    ...
    product_id = fields.Function(fields.One2Many('product.product',None,'product_id',depends=['product_id','drug_type']),'on_change_with_product_id')
...
class InternalApplyList(ModelView):
    'Internal Apply List'
    __name__ = 'hrp_internal_delivery.internal_apply_list'
    _rec_name = 'number'
    ...
    moves = fields.One2Many('hrp_internal_delivery.internal_apply_modify','None','Moves')
                    ...
                    if TestInternal:
                        ...
                        for i in TestInternal:
                            ...
                            for each in Move:
                                ...
                                dict['lot'] = each.lot#退药的批次
                                with Transaction().set_context(stock_date_end=Date.today()):
                                    quantities = Product.products_by_location([self.to_location.id], [each.product.id], with_childs=True)
                                if quantities.values():
                                    stock_level = [v for v in quantities.values()][0]
                                    dict['stock_level'] = stock_level
                                else:
...
class ApplyModifyWizard(Wizard):
    'Apply Modify Wizard'

    __name__ = 'hrp_internal_delivery.apply_modify_wizard'

    start = StateView('hrp_internal_delivery.internal_apply_list',
        'hrp_internal_delivery.internal_apply_list_view_form', [
    ...
    def do_create_(self, action):
        ...
        for each in Move:
            if each['message_confirm'] == True:
                shipment_id = each['shipment_id']
                Number = each['move_number']
                ...
                if internal:
                    UserId = Pool().get('hrp_internal_delivery.test_straight')
                    if UserId.get_user_id() == config.warehouse.storage_location.id:
                        product_id = []
                        for pro_id in internal:
                            for u_id in pro_id.moves:
                                product_id.append(u_id.product.id)
                        line_item = each['moves']
                        lin_product_id = []
                        for i in line_item:
                            lin_product_id.append(i['product_choice'])
                        if set(product_id).issubset(set(lin_product_id)):
                            Internal.delete(internal)
                        else:
                            self.raise_user_error(u'不允许删除药品，请您退出该界面重新修改')
                    else:
                        Internal.delete(internal)
                else:
                    self.raise_user_error(u'该请领单已发药完成',u'不允许修改')#错误信息弹框提示
                line_item = each['moves']
                lv = {}
                list = []
                lv['starts'] = u'00'
                ...
                lv['from_location'] = config.warehouse.storage_location.id
                lv['planned_date'] = Date.today()
                lv['place_of_service'] = data['start']['moves'][num]['location']
                lv['state'] = u'draft'
                lv['number'] = Number
                lv['drug_starts'] = data['start']['moves'][0]['drug_type']
                for i in line_item:
                    dict = {}
                    dict['origin'] = None  # each['origin']
                    dict['to_location'] = config.transfers.id  # 中转库
                    dict['product'] = i['product_choice']
                    dict['from_location'] = config.warehouse.storage_location.id
